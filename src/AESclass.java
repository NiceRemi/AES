import java.math.BigInteger;

public class AESclass {
	static int Nr=10;
	static int key[]={0x2B,0x7E,0x15,0x16,0x28,0xAE,0xD2,0xA6,0xAB,0xF7,0x15,0x88,0x09,0xCF,0x4F,0x3C};
	static int text[]={0x32,0x43,0xF6,0xA8,0x88,0x5A,0x30,0x8D,0x31,0x31,0x98,0xA2,0xE0,0x37,0x07,0x34};
	static int sbox[]={
			, 
			};
	static int RCon[];
	static int word[][];
	public static void main(String[] args) {
//		keyExpansion();
//		int i,r=0;
//		text=addRoundKey(text,0);
//		System.out.println("No.0 add roundKey:");
//		for(i=0;i<16;i++){
//			System.out.print(Integer.toHexString(text[i])+',');
//			if(i%4==3){
//				System.out.println();
//			}
//		}
//		System.out.println();
//		r++;
//		for(;r<=Nr;r++){
//			for(i=0;i<16;i++){
//				text[i]=subByte(text[i]);
//			}
//			System.out.println("subByte:");
//			for(i=0;i<16;i++){
//				System.out.print(Integer.toHexString(text[i])+',');
//				if(i%4==3){
//					System.out.println();
//				}
//			}
//			text=shiftRows(text);
//			System.out.println("No."+r+" shiftRows:");
//			for(i=0;i<16;i++){
//				System.out.print(Integer.toHexString(text[i])+',');
//				if(i%4==3){
//					System.out.println();
//				}
//			}
//			if(r!=10){
//				text=mixColumn(text);
//				System.out.println("No."+r+" mixColumn:");
//				for(i=0;i<16;i++){
//					System.out.print(Integer.toHexString(text[i])+',');
//					if(i%4==3){
//						System.out.println();
//					}
//				}
//			}
//			text=addRoundKey(text, r);
//			System.out.println("No."+r+" add roundKey:");
//			for(i=0;i<16;i++){
//				System.out.print(Integer.toHexString(text[i])+',');
//				if(i%4==3){
//					System.out.println();
//				}
//			}
//			System.out.println();
//		}
		
		System.out.println(Integer.toHexString(fieldMulit(2, new BigInteger("E6",16).intValue())));
	}
	static void keyExpansion(){
		RCon=new int[10];
		RCon[0]=0x01;  
	    RCon[1]=0x02;  
	    RCon[2]=0x04;  
	    RCon[3]=0x08;  
	    RCon[4]=0x10;  
	    RCon[5]=0x20;  
		RCon[6]=0x40;  
		RCon[7]=0x80;  
		RCon[8]=0x1B;  
		RCon[9]=0x36; 
		word=new int [44][4];
		int i,j=0;
		int temp[]=new int[4];
		for(i=0;i<4;i++){
			for(j=0;j<4;j++){
				word[i][j]=key[i*4+j];
			}
		}
		for(i=4;i<44;i++){
			temp=word[i-1];
			if(i%4==0){
				temp=subWord(rotWord(temp));
				temp[0]=temp[0]^RCon[i/4-1];
			}
			for(j=0;j<4;j++){
				word[i][j]=word[i-4][j]^temp[j];
			}
		}
	}
	static int[] rotWord(int[] word){
		int[] rot=new int[4];
		int i;
		for(i=0;i<4;i++){
			rot[i]=word[(i+1)%4];
		}
		return rot;
	}
	static int[] subWord(int[] word){
		int sub[]=new int[4];
		int i;
		for(i=0;i<4;i++){
			sub[i]=subByte(word[i]);
		}
		return sub;
	}
	static int subByte(int w){
		int x=w/16;
		int y=w%16;
		return sbox[x*16+y];
	}
	static int[] addRoundKey(int[]text,int round){
		int[] add=new int[16];
		int i,j;
		System.out.println("NO."+round+" roundKey:");
		for(i=0;i<4;i++){
			for(j=0;j<4;j++){
				System.out.print(Integer.toHexString(word[4*round+i][j])+",");
				add[4*i+j]=text[4*i+j]^word[4*round+i][j];
			}
			System.out.println();
		}
		return add;
	}
	static int[] shiftRows(int[] text){
		int[] shift=new int[16];
		int i,j;
		for (i=0;i<4;i++){
			for(j=0;j<4;j++){
				shift[4*i+j]=text[4*((i+j)%4)+j];
			}
		}
		return shift;
	}
	static int fieldMulit(int x,int y){
		String xString=Integer.toBinaryString(x);
		int i,j,mul=0,tem=y;
		for (i=0;i<xString.length();i++){
			if(xString.charAt(i)=='1'){
				for(j=1;j<xString.length()-i;j++){
					tem=tem<<1;
					if(tem>255){
						tem=tem%256;
						tem=tem^0x1b;
					}
				}
				mul=mul^tem;
				tem=y;
			}
		}
		return mul;
	}
	static int[] mixColumn(int[] text){
		int[] mix=new int[16];
		int[] mass={2,3,1,1};
		int i,j,u;
		for(i=0;i<16;i++){
			u=0;
			for (j=0;j<4;j++){
				u=u^fieldMulit(text[(i/4)*4+j],mass[(4-i%4+j)%4]);
			}
			mix[i]=u;
		}
		return mix;
	}
}